...
class ReadLatestActivityTest extends TestCase
{
    ...
    public function a_user_can_read_only_the_posting_activities()
    {
        ...
        createMany(Thread::class, $numberOfPostingActivities, ['user_id' => $profileOwner->id]);

        $thread = Thread::first();

        $reply = $thread->addReply(raw(Reply::class, ['user_id' => $profileOwner->id]));

        $numberOfPostingActivities++;

        $reply->likedBy($profileOwner);

        $response = $this->get(route('api.latest-activity.index', [$profileOwner, 'postings' => true]))->json();

        $this->assertCount($numberOfPostingActivities, $response['data']);
    }
}